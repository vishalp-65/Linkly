...
class RedisConnection {
                    ...
                    socket: {
                        reconnectStrategy: (retries: number) => {
                            if (retries > config.maxRetries) {
                                logger.error(
                                    "Redis cluster max retries exceeded"
                                )
                                return false
                            }
    ...
    private setupEventHandlers(): void {
        this.client.on("connect", () => {
            logger.info(
                `Redis ${this.isCluster ? "cluster" : "client"} connecting`
            )
        })
        ...
        this.client.on("error", (error: any) => {
            logger.error(
                `Redis ${this.isCluster ? "cluster" : "client"} error`,
                {
                    error: error.message,
                    stack: error.stack
                }
        ...
        this.client.on("reconnecting", () => {
            logger.warn(
                `Redis ${this.isCluster ? "cluster" : "client"} reconnecting`
            )
        })
    }
    ...
    public async keys(pattern: string): Promise<string[]> {
        try {
            if (this.isCluster) {
                // Cluster mode: manually iterate over masters and collect keys
                const cluster = this.client as RedisClusterType
                const masters = cluster.masters
                    ? cluster.masters
                    : await (cluster as any).nodes("master")

                const allKeys: string[] = []
                for (const master of masters) {
                    const nodeKeys = await master.client.keys(pattern)
                    allKeys.push(...nodeKeys)
                }

                logger.debug("Redis Cluster KEYS operation", {
                    pattern,
