...
export function getCountryName(countryCode: string | null | undefined): string {
    // Handle null, undefined, empty string, or explicit "Unknown"
    if (!countryCode ||
        countryCode === 'Unknown' ||
        countryCode === 'unknown' ||
        countryCode === 'XX' ||
    ...
    // If it's already a country name (longer than 2 chars), return it
    if (upperCode.length > 2) {
        return countryCode;
    }

    return countryCodeToName[upperCode] || 'Unknown';
}

/**
 * Get country flag emoji from country code
 * @param countryCode - ISO 3166-1 alpha-2 country code
 * @returns Flag emoji or globe emoji for unknown
 */
export function getCountryFlag(countryCode: string | null | undefined): string {
    ...
    const codePoints = [...upperCode].map(char =>
