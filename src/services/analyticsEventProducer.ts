...
import { AnalyticsEvent, CreateAnalyticsEventInput } from '../types/database';
...
class AnalyticsEventProducer {
    ...
    private async flushBuffer(): Promise<void> {
        if (this.eventBuffer.length === 0 || !this.isProducerReady || !this.producer) {
            return;
        }
            ...
            if (!this.isProducerReady) {
                setTimeout(() => {
                    this.initializeProducer().catch(retryError => {
                        logger.error('Failed to retry producer initialization', {
    ...
    public getBufferStats(): { bufferSize: number; maxBufferSize: number; isProducerReady: boolean } {
        return {
